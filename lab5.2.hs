...
filterSteps x = let 
                    ...
                    p8 x = elem 8 x
                    p9 x = elem 9 x
                    setR = (if p2 x then [2] else []) ++ (if p4 x then [4] else []) ++ (if p6 x then [6] else []) ++ (if p8 x then [8] else [])
                    setS = (if p1 x then [1] else []) ++ (if p3 x then [3] else []) ++ (if p5 x then [5] else []) ++ (if p7 x then [7] else []) ++ (if p9 x then [9] else [])
                   in
                    if length setS > 1 then setS else if length setR == 0 then setS else setR
...
game s ai = do
    if isWin s then do
        ...
        if length (getFrees s) == 0 then do
            ...
            if ai then do
                if length (getFrees s) == 8 && elem 5 (getFrees s) then do
                    let s2 = play 5 s
                    printField s2
                    game s2 False
                 else do
                    let steps = filterSteps (bestSteps s)
